...
<?php return ['SUPABASE_URL' => 'http://127.0.0.1:8131/supabase', 'SUPABASE_ANON' => 'anon',
  ...
  'TIKTOK_ACCESS_TOKEN' => 'tt-test-token', 'TIKTOK_API_BASE' => 'http://127.0.0.1:8131/tiktok'];
PHP
...
python3 /tmp/claude-0/paytest/test.py                                        # 113 проверок
```
...
Мок умеет `POST /__set` с флагами: `fail_create [код, error_code]`, `invoice_get_fail`, `no_id`, `fail_rpc`, `fail_wa`, `fail_mail`, `fail_tt` (TikTok Events API).
