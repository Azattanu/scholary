...
create table if not exists portfolio_items (id bigint generated always as identity primary key, user_id uuid not null references auth.users(id) on delete cascade, program_id text not null, status text default 'study', note text, added_at timestamptz default now(), submitted_at timestamptz, outcome text, readiness int default 0, checklist jsonb default '{}', unique (user_id, program_id));
